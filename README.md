...
| 保洁巡查 | `/inspections` | 按班次配置检查项组合、逐项打分、自动折算百分制得分与等级、班次/日期/结论筛选、巡查详情、一键转问题上报 |
...
| GET | `/stats/shift-comparison` | 跨班次均分对比（原始均分 + 按可比项目折算的均分，附折算规则说明） |
| GET | `/meta/dictionaries` | 枚举字典（状态、分类、程度、检查项池、各班次当前组合、流转规则） |
| GET | `/meta/checklists` | 各班次当前检查项组合（含版本号） |
| PUT | `/meta/checklists/{shift}` | 调整班次组合，生成新版本，只对之后的录入生效 |
| GET | `/meta/checklists/history` | 班次组合历史版本 |
| GET | `/meta/restroom-options` | 公厕下拉选项 |
| GET | `/health` | 健康检查 |

## 业务规则

- **检查项组合**：全部可选检查项构成检查项池（8 项，每项 0-10 分），早班/中班/晚班各自配置池的子集作为检查项组合（早班侧重开放前全面保洁与耗材就位，中班侧重高峰维持与异味控制，晚班侧重收班清运与设施安全）。组合按版本管理：每次调整生成新版本，**只对之后录入的巡查生效**；录入时提交的检查项必须与该班次当前组合完全一致（缺项、含组合外项目都会被拒绝），同一条巡查整体按同一个组合版本展开，不会一半新一半旧。
- **巡查评分**：每条巡查按提交时绑定的组合版本展开检查项，得分 = 总得分 / 满分 × 100；≥90 优秀、≥80 良好、≥70 合格，其余不合格。任一检查项低于 6 分或等级为不合格时，巡查结论自动置为「发现问题」。历史记录的组合版本、明细与得分保持不变，仍按提交当时的组合展示和算分。
- **跨班次均分对比**：可比项目 = 三个班次现行组合的交集（只有所有班次共同检查的项目才具备跨班次可比性）。比较时仅取每条记录中属于可比项目的打分折算百分制再求平均，不含可比项目的记录不计入。可比项目集合由当前组合配置唯一确定，同一批数据只会折算出一个均分；折算规则说明随 `/stats/shift-comparison` 接口一并返回，看板「班次均分对比」面板同步展示。
